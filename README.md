...
マイナカードの処理にあたり, wineに対して以下のpipe-peek.patchが必要になります.
...
以下のようにマイナポータルアプリのインストールができます. 多少文字化けがありますが, がんばりましょう.

```
# 別環境を作っておくとべんりかもしれない
$ export WINEPREFIX=$HOME/.local/share/wineprefixes/mnp-test
...
cd "${WINEPREFIX}/drive_c/users/<user>/AppData/Local/MPA/Chrome/extension"
tee "${LOGDIR}/in.log" |
